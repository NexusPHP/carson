...
> [!NOTE]
> `pull_request_review` and `pull_request_review_comment` have no `_target` variant, so on a pull request from a fork GitHub runs them without secrets and Carson cannot authenticate. Carson detects this case (both credentials empty, one of those events or `pull_request`, head repository different from the base) and ends the run successfully with a notice instead of failing. Review activity on a fork pull request is therefore not seen when it happens. Subscribers that depend on it catch up on the pull request's next `pull_request_target` event, such as a push, and `triage-labeler` can reconcile open pull requests on each scheduled run with its `sweep` setting. Empty credentials in any other situation still fail the run.
