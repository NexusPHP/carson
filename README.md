...
jobs:
  carson:
    ...
    steps:
      - uses: NexusPHP/carson@<commit-sha>  # v1.4.3
        with:
...
Pin to a specific commit SHA and let [Dependabot](https://docs.github.com/en/code-security/dependabot/dependabot-version-updates) keep it updated via the trailing `# v1.4.3` comment. Carson does not maintain a moving `v1` tag: every release is a fixed `vX.Y.Z`.
