...
Event-driven comments go through `this.notice(context, number, body)` ([src/subscriber.ts](src/subscriber.ts)), which queues the text. `Carson.run` wraps `probot.webhooks.receive` so the queue is flushed once every handler for the event has finished ([src/github/notices.ts](src/github/notices.ts)): a lone notice posts as a plain comment ending with `<!-- carson:<id> -->`, two or more on the same item post as one digest ending with `<!-- carson:digest -->`. Find a prior notice with `findNotice(comments, this.id, isBotComment)` (or `isBotNode` for GraphQL), which matches both shapes and returns the latest, and use `this.resolveNotice` / `this.reopenNotice`, which minimize a standalone comment but only collapse or expand the section of a digest, re-reading the live body first. Scheduled passes that comment per item (`stale`, `no-response-closer`, `lock-old-issues`, the `draft-policy` close) call `createComment` directly: their farewell comments must not carry a marker, or `findNotice` would mistake them for notices. When adding a subscriber that posts trackable comments, list its marker in the table in [SUBSCRIBERS.md](SUBSCRIBERS.md#comment-markers).
