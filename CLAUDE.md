...
[src/carson.ts](src/carson.ts) holds a list of `Subscriber` instances and wires them up on `run(probot)`. Each `Subscriber` ([src/subscriber.ts](src/subscriber.ts)) implements:
